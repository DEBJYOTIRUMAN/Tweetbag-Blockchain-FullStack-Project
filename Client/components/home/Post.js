...
const Post = ({ post, currentAccount, currentUser, posts, setPosts }) => {
  ...
  const likePost = async () => {
    ...
    let url = `https://tweetbag.onrender.com/api/post/like/${post._id}`;
    if (pathName === "/explore") {
      url = `https://tweetbag.onrender.com/api/news/like/${post._id}`;
    }
    fetch(url, {
      method: "PUT",
      headers: {
  ...
  const handleRepost = async () => {
    ...
    fetch(`https://tweetbag.onrender.com/api/post/repost/${post._id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const deletePost = async () => {
    fetch(`https://tweetbag.onrender.com/api/post/${post._id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((document) => {
    ...
    Object.keys(post).length !== 0 ? (
      <>
        <div className={style.wrapper}>
          <div
            onClick={() => router.push(`/profile/${post.walletAddress}`)}
            className="sm:mx-3"
          >
            <img
            ...
            <div className={style.footer}>
              {pathName !== "/explore" ? (
                <>
                  ...
                  {post.walletAddress === currentAccount ? (
                    <div
                      className={style.wrapFooterIcon}
                      onClick={() => deletePost()}
                    >
