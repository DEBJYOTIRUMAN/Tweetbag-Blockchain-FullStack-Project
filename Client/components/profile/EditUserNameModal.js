...
const EditUserNameModal = ({
  ...
  const updateUserName = () => {
    if (!name) return;
    fetch(`https://tweetbag.onrender.com/api/user/${currentAccount}`, {
      method: "PUT",
      headers: {
      ...
      .then((userDoc) => {
        ...
        setAllUsers(
          allUsers.map((user) =>
            user.walletAddress === currentAccount ? userDoc : user
          )
        );
  ...
  return (
    <div className={style.wrapper}>
      ...
      <div className={style.lower}>
        ...
        <div
          className={name ? style.mintButton : style.inactiveMintButton}
